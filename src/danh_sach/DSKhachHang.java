...
public class DSKhachHang implements DanhSach<KhachHang> {
    ...
    public void thongKe() {
        ...
        String[] diaChiArray = new String[n];
        for (int i = 0; i < n; i++) {
            diaChiArray[i] = dskh[i].getDiaChi();
        }
        Arrays.sort(diaChiArray);
        System.out.println("Thong ke theo dia chi:");
        int count = 1;
        for (int i = 1; i < n; i++) {
            if (diaChiArray[i].equals(diaChiArray[i - 1])) {
                count++;
            } else {
                System.out.println("Dia chi: " + diaChiArray[i - 1] + ", So luong: " + count);
                count = 1;
            }
        }
        System.out.println("Dia chi: " + diaChiArray[n - 1] + ", So luong: " + count);
    }
}
